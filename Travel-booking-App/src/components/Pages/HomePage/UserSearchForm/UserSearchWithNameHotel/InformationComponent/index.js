...
export const HotelsDetailWithSearch = () => {
  ...
  const handleFetchHotelListDetail = async () => {
    ...
    const data = dataHotel.filter((item) => {
      return item.name === params.searchWithNameHotel;
    });
    // console.log(data);
    setHotelDetail(data);
  ...
  const handleFetchCmt = async () => {
    const response = await fetch(
      `https://6268162901dab900f1c9969b.mockapi.io/appi/v1/comments?idHotel=${hotelDetails.id}`
    );
          ...
          <div className="detailSubContainer">
            <div className="detailFirstContainer">
              <div>
                <img
                  className="mainImg"
                  src={require(`` + hotelDetail.img1 + ``)}
                />
                <img
                  className="subImg"
                  src={require(`` + hotelDetail.img2 + ``)}
                />
                <img
                  className="subImg"
                  src={require(`` + hotelDetail.img3 + ``)}
                />
                <img
                  className="subImg"
                  src={require(`` + hotelDetail.img4 + ``)}
                />
              </div>
            ...
            <div className="convenientContainer">
              Tiện ích
              {ConvenientList.filter((icon) =>
                hotelDetail.convenient.includes(icon.name)
              ).map((icon) => {
                return (
                  <div>
            ...
            <div>
              ...
              <div>
                <div className="detailRoomCard">
                  <div>
                    <img
                      className="detailRoomImg"
                      src={require(`` + hotelDetail.room1Img + ``)}
                    />
                  </div>
                  <div>
                ...
                <div className="detailRoomCard">
                  <div>
                    <img
                      className="detailRoomImg"
                      src={require(`` + hotelDetail.room2Img + ``)}
                    />
                  </div>
                  <div>
